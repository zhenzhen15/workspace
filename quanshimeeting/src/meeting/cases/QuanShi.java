...
public class QuanShi extends UiAutomatorTestCase{
	public void testCase() throws UiObjectNotFoundException, IOException, RemoteException{
		    FindAssistant findAssistant= FindAssistant.getInstance();
		    //启动app
			Runtime.getRuntime().exec("am start -S com.gnet.onemeeting/com.gnet.onemeeting.LaunchActivity");
			sleep(5000);
			
			//点击登录
			FindAssistant.ClickByText("登录");
			
			//点击logo 进入环境选择页面
			findAssistant.ClickById("com.gnet.onemeeting:id/iv_default_logo");
			
			//选择环境
			FindAssistant.ClickByClassName("android.widget.RadioButton",0);
			
			//点击确定，进入登录页面
			
			FindAssistant.ClickByText("确定");
